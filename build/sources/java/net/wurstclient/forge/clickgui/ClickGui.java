...
public final class ClickGui
{
	...
	private float[] bgColor = {0.0625F, 0.0625F, 0.0625F};
	private float[] acColor = {0.0F, 0.5F, 1.0F};
	private float opacity = 0.75F;
	private int maxHeight = 250;

	private String tooltip;

	public ClickGui(HackList hackList)
	{
		this.hackList = hackList;
		Path configFolder = ForgeWurst.getForgeWurst().getConfigFolder();
		windowsFile = configFolder.resolve("windows.json");
		init(hackList);
	}

	private void init(HackList hax)
	{
		...
		Hack clickGuiHack = hax.get("ClickGUI");
		if(clickGuiHack != null && !clickGuiHack.getSettings().isEmpty())
		{
			Window uiSettings = new Window("UI Settings");
			for(Setting setting : clickGuiHack.getSettings().values())
	...
	private void tryAddSpecialWindow(String hackName)
	{
		Hack hack = hackList.get(hackName);
		if(hack == null)
			return;

		try
		{
			Method getWindow = hack.getClass().getMethod("getWindow");
			Object result = getWindow.invoke(hack);
	...
	private void loadWindows()
	{
		...
		for(Window window : windows)
		{
			...
			JsonElement jsonPinned = object.get("pinned");
	...
	public void open()
	{
		Minecraft.getMinecraft().displayGuiScreen(new ClickGuiScreen(this));
	}
	...
	private void handleScrollbarMouseClick(Window window, int mouseX, int mouseY,
		int mouseButton)
	{
		if(mouseButton != 0 || mouseX >= window.getWidth() - 1)
			return;
	...
	private void handleComponentMouseClick(Window window, int mouseX, int mouseY,
		int mouseButton)
	{
		for(int i = window.countChildren() - 1; i >= 0; i--)
		{
			Component component = window.getChild(i);
			if(mouseX < component.getX() || mouseY < component.getY())
				continue;
			if(mouseX >= component.getX() + component.getWidth()
				|| mouseY >= component.getY() + component.getHeight())
				continue;

			component.handleMouseClick(mouseX, mouseY, mouseButton);
			break;
		}
	}
	...
	public void render(int mouseX, int mouseY, float partialTicks)
	{
		...
		for(Popup popup : popups)
		{
			...
			popup.render(mouseX - x1, mouseY - y1);
			GL11.glPopMatrix();
		}
	...
	private void renderTooltip(int mouseX, int mouseY)
	{
		...
		for(String line : lines)
			tw = Math.max(tw, fr.getStringWidth(line));

		int sw = mc.currentScreen.width;
		int sh = mc.currentScreen.height;
		int x1 = mouseX + tw + 11 <= sw ? mouseX + 8 : mouseX - tw - 8;
		int x2 = x1 + tw + 3;
		int y1 = mouseY + th - 2 <= sh ? mouseY - 4 : mouseY - th - 4;
		int y2 = y1 + th + 2;
		...
		for(int i = 0; i < lines.length; i++)
			fr.drawString(lines[i], x1 + 2, y1 + 2 + i * fr.FONT_HEIGHT,
				0xffffff);
	}
	...
	{
		bgColor = new float[] {0.0625F, 0.0625F, 0.0625F};
		acColor = new float[] {0.0F, 0.5F, 1.0F};
		opacity = 0.75F;
		maxHeight = 250;

		Hack clickGuiHack = hackList.get("ClickGUI");
		if(clickGuiHack != null)
		{
			opacity = readFloatMethod(clickGuiHack, "getOpacity", opacity);
			maxHeight = (int)readFloatMethod(clickGuiHack, "getMaxHeight",
				maxHeight);

			float[] reflectedBg =
				readColorMethod(clickGuiHack, "getBgColor", bgColor);
			float[] reflectedAc =
				readColorMethod(clickGuiHack, "getAcColor", acColor);
			bgColor = reflectedBg;
			acColor = reflectedAc;

			readLegacyClickGuiSettings(clickGuiHack);
		}
		...
		if(rainbowUiHack != null && rainbowUiHack.isEnabled())
		{
			...
			acColor[1] = 0.5F
				+ 0.5F * (float)Math.sin((x + 4F / 3F) * Math.PI);
			acColor[2] = 0.5F
				+ 0.5F * (float)Math.sin((x + 8F / 3F) * Math.PI);
		}
	}

	private void readLegacyClickGuiSettings(Hack clickGuiHack)
	{
		for(Setting setting : clickGuiHack.getSettings().values())
		{
			...
			if(name.equals("opacity"))
			{
				...
				opacity = value > 1 ? (float)(value / 100.0) : slider.getValueF();
			}else if(name.equals("max height") || name.equals("maxheight"))
				maxHeight = slider.getValueI();
		}
	}

	private float readFloatMethod(Hack hack, String name, float fallback)
	{
		try
		{
			Method method = hack.getClass().getMethod(name);
			Object result = method.invoke(hack);
	...
	private float[] readColorMethod(Hack hack, String name, float[] fallback)
	{
		try
		{
			Method method = hack.getClass().getMethod(name);
			Object result = method.invoke(hack);
			if(result instanceof float[] && ((float[])result).length >= 3)
		...
		float partialTicks)
	{
		...
		if(!window.isMinimized())
		{
			...
			if(window.isScrollingEnabled())
			{
				...
				boolean hovering = mouseX >= xs1 && mouseY >= ys3 && mouseX < xs2
					&& mouseY < ys4;
		...
		String title = fontRenderer.trimStringToWidth(window.getTitle(),
			Math.max(0, x3 - x1 - 2));
