...
public abstract class WurstClassVisitor extends ClassVisitor
{
	...
	{
		return FMLDeobfuscatingRemapper.INSTANCE.mapMethodName(owner, name,
			desc);
	}

	protected String mapFieldName(String owner, String name, String desc)
	{
		return FMLDeobfuscatingRemapper.INSTANCE.mapFieldName(owner, name,
			desc);
	}
	...
	private static final class MethodVisitorRegistryEntry
	{
		...
		private MethodVisitorRegistryEntry(String name, String desc,
			MethodVisitorFactory factory)
		{
			this.name = name;
			this.desc = desc;
