...
public final class Radar extends Component
{
	...
	public void render(int mouseX, int mouseY, float partialTicks)
	{
		...
		for(Entity entity : hack.getEntities())
		{
			double diffX = entity.prevPosX
				+ (entity.posX - entity.prevPosX) * partialTicks
				- (player.prevPosX + (player.posX - player.prevPosX)
					* partialTicks);
			double diffZ = entity.prevPosZ
			...
			double angle = hack.isRotateEnabled()
				? Math.toRadians(player.rotationYaw - neededRotation - 90)
				: Math.toRadians(180 - neededRotation - 90);
